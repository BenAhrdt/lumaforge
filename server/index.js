...
const api = async(req,res,url)=>{
  if(req.method==='GET'&&url.pathname==='/api/v1/status'){const now=process.hrtime.bigint(),usage=process.cpuUsage(previousCpuUsage),elapsed=Number(now-previousCpuAt)/1000;previousCpuUsage=process.cpuUsage();previousCpuAt=now;const memory=process.memoryUsage();return json(res,200,{version:'0.1.4-dev',cpuPercent:Math.min(100,(usage.user+usage.system)/Math.max(1,elapsed)*100),memoryUsedBytes:memory.heapUsed,memoryTotalBytes:memory.heapTotal});}
