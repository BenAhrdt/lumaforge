...
const api = async(req,res,url)=>{
  ...
  if(req.method==='GET'&&url.pathname==='/api/v1/info')return json(res,200,{product:'LumaForge',device_id:'lf-000000000000',device_name:project.device.name,model:'simulator',firmware_version:'0.2.0-alpha.1',api_version:1,hostname:'localhost',network:{connected:true,ip:'127.0.0.1'},capabilities:['led_output','scenes','zones']});
  if(req.method==='PUT'&&url.pathname==='/api/v1/device'){const value=await body(req),name=String(value.device_name||'').trim();if(!name||name.length>40)return json(res,422,{error:'device_name must contain 1 to 40 characters'});project.device.name=name;await storage.write('device',project.device);return json(res,200,{device_id:'lf-000000000000',device_name:name});}
