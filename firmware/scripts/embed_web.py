...
for index, name in enumerate(assets):
    ...
    if name == "app.js":
        ...
        content += '''\nconst renderFirmwareDeviceSettings=renderDeviceSettings;renderDeviceSettings=function(){renderFirmwareDeviceSettings();const panel=document.querySelector('#device-settings');if(panel&&!panel.querySelector('.firmware-network-settings')){const network=document.createElement('button');network.className='wide firmware-network-settings';network.textContent=state.language==='de'?'WLAN und Gerätenamen ändern':'Change Wi-Fi and device name';network.onclick=()=>location.href='/setup';const update=document.createElement('button');update.className='wide firmware-update';update.textContent=state.language==='de'?'Firmware aktualisieren (OTA)':'Update firmware (OTA)';update.onclick=()=>location.href='/update';panel.append(network,update);}};\n'''.encode("utf-8")
